...
void World::build(){
	...
	camera_ptr->set_eye(50, 50, -75);
	camera_ptr->set_lookat(0, 0, 0);
	camera_ptr->set_view_distance(4500);
	camera_ptr->set_roll_angle(0); //rotate camera
	camera_ptr->compute_uvw();
	set_camera(camera_ptr);

	Emissive* emissive_ptr1 = new Emissive;
	emissive_ptr1->scale_radiance(20.0);
	emissive_ptr1->set_ce(red);

	MultiJittered* area_sampler_ptr1 = new MultiJittered(num_samples);

	Rectangle* rectangle_ptr1 = new Rectangle(Point3D(0, 0, -2), Vector3D(2, 0, 0),Vector3D(0, 2, 0));
	...
	matte_ptr1->set_cd(RGBColor(1, 1, 1)); // blue
